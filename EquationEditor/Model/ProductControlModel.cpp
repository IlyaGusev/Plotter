...
void CProductControlModel::updatePolygons()
{
	...
	params.polygon.push_back( CLine( piLeft, piTop, piLeft, piBottom ) ); // лева€ палка
	params.polygon.push_back( CLine( piRight, piTop, piRight, piBottom ) ); // права€ палка
	params.polygon.push_back( CLine( rect.Left(), piTop, rect.Right(), piTop ) ); // верхн€€ перекладина
}
